...
class TestJWTFunctions(unittest.TestCase):
    ...
    def test_create_and_verify_client_jwt(self):
        ...
        self.assertTrue(
            JwtUtils.VerifyJwt(token, self.supabase_service),
            "Client JWT should be valid",
        )
    ...
    def test_create_and_verify_user_jwt(self):
        ...
        self.assertTrue(
            JwtUtils.VerifyJwt(token, self.supabase_service), "User JWT should be valid"
        )
        decoded = jwt.decode(token, JWT_SECRET, algorithms=["HS256"])
        self.assertEqual(decoded["sub"], self.username)
        self.assertEqual(decoded["type"], "user")
        self.assertNotIn(
            "client_secret", decoded, "User JWT should not have client_secret"
        )
    ...
    def test_expired_client_jwt(self):
        ...
        self.assertFalse(
            JwtUtils.VerifyJwt(token, self.supabase_service),
            "Expired client JWT should be invalid",
        )
    ...
    def test_expired_user_jwt(self):
        ...
        self.assertFalse(
            JwtUtils.VerifyJwt(token, self.supabase_service),
            "Expired user JWT should be invalid",
        )

    def test_invalid_token(self):
        token = (
            JwtUtils.CreateClientJwt(self.client_id, self.client_secret) + "tampered"
        )
        self.assertFalse(
            JwtUtils.VerifyJwt(token, self.supabase_service),
    ...
    def test_inactive_client_jwt(self):
        token = JwtUtils.CreateClientJwt(
            self.inactive_client_id, self.inactive_client_secret
        )
        self.assertFalse(
            JwtUtils.VerifyJwt(token, self.supabase_service),
            "Inactive client JWT should be invalid",
        )
    ...
    def test_inactive_user_jwt(self):
        ...
        self.assertFalse(
            JwtUtils.VerifyJwt(token, self.supabase_service),
            "Inactive user JWT should be invalid",
        )
